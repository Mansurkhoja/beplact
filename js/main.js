'use strict'; // JUST SLIDER SWIPER FOR CATALO SECTION
...
var mixItems = document.getElementById('mix-move');
var mixItem = mixItems.querySelectorAll('.mix__move');
mixItems.addEventListener('mousemove', function (e) {
  var x = e.pageX - this.offsetLeft;
  var y = e.pageY - this.offsetTop;
  var s = this.clientWidth / 2;
  var ss = this.clientHeight / 2;
  mixItem.forEach(function (element) {
    element.style.transform = "translate(".concat(x - s, "px, ").concat(y - ss, "px)");
  });
});
mixItems.addEventListener('mouseleave', function () {
  mixItem.forEach(function (element) {
    element.style.transform = "translate(".concat(0, "px, ", 0, "px)");
  });
});
mixItems.addEventListener('touchend', function (event) {
